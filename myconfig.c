...
#define D_CHARGEMODE        1   /*默认使用收费资源*/
...
unsigned chargeMode = D_CHARGEMODE; /*计费规则，0为免费资源，1为收费资源*/
...
static int readFile(int *daemonMode)
{
	...
	*daemonMode = getInt(buf, "MentoHUST", "DaemonMode", D_DAEMONMODE) % 4;
	...
	chargeMode = getInt(buf, "MentoHUST", "ChargeMode", D_CHARGEMODE) % 2;
	free(buf);
	return 0;
}
...
static int getAdapter()
{
	...
	for (d=alldevs; i>1; d=d->next, i--);// 将d指向选择的网卡
...
static void saveConfig(int daemonMode)
{
	...
	setInt(&buf, "MentoHUST", "ChargeMode", chargeMode);
