...
int bufType = 1;	/*0内置xrgsu 1内置Win 2仅文件 3文件+校验*/
u_char version[2];	/* 版本 */
#ifndef NO_ARP
...
extern unsigned startMode, dhcpMode, chargeMode;
...
char *gbk2utf(char *src, size_t srclen) {
	...
	str_normalize_init();
    const char *gbk = src;
    uint32_t gbk_len = srclen;
    uint32_t utf8buffer_len = srclen * 3 + 1;
...
void fillStartPacket()
{
	if (bufType <= 1) {	/* 使用xrgsu？ */
		...
		const u_char packet1Free[] = {
			...
			0x36,0x39,0x33,0x61,0x34,0x66,0x35,0x61,0x32,0x39,0x32,0x38,0x36,0x37,0x35,0x31,
			0x66,0x39,0x37,0x66,0x34,0x64,0x30,0x34,0x36,0x38,0x1a,0x28,0x00,0x00,0x13,0x11,
			0x39,0x22,0xc3,0xe2,0xb7,0xd1,0xd7,0xca,0xd4,0xb4,0x00,0x00,0x00,0x00,0x00,0x00,
			0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
			0x00,0x00,0x1a,0x48,0x00,0x00,0x13,0x11,0x54,0x42,0x48,0x55,0x53,0x54,0x4d,0x4f,
			0x4f,0x4e,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
			0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
		...
		if (bufType == 1) {
			if (chargeMode == 1){
				printf("fill packet 收费");
				memcpy(fillBuf+0x17, packet1, sizeof(packet1));// 收费
			}
			else {
				printf("fill packet 免费");
				memcpy(fillBuf+0x17, packet1Free, sizeof(packet1Free));// 免费
			}
			memcpy(fillBuf+0x3b, version, 2);
		} else {
			printf("bufType = %d\n", bufType);
